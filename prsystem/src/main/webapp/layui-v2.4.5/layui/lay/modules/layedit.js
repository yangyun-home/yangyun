...
layui.define(["layer", "form","jquery"], function(t) {
	"use strict";
	var e = layui.$,
		i = layui.layer,
		a = layui.form,
	...
	var s = function(t, i, a) {
				...
				c = {
						...
						layui.use("upload", function(o) {
							...
							o.render({
								...
								before: function(obj) {
									...
									var resultData = $.parseJSON(e.identResult);
									console.log(resultData.conclusion,typeof resultData.conclusion);
									
									if(e.code == 0){
										if("正常"==resultData.conclusion){
											(e.data = e.data || {}, v.call(t, "img", {
												src: e.data.src,
												alt: e.data.title,
												style: 'display:inline-block;max-width:90%;height:auto'
											}, a))
										}else{
											i.msg("该图片含有"+resultData.conclusion+"信息，请重新选择上传", {time:2000, icon:5, shift:6}, function(){
					...
					preview:function(){
						...
						/*i.open({
							...
							area: ["600px", "380px"],
							shade: .1,
							content:document.getElementsByTagName('iframe')[0].innerHTML
						})*/
					}
				},
		...
		C = {
			...
			preview: '<i class="layui-icon layui-icon-read" title="预览" layedit-event="preview"></i>',
