...
<div style="display: none;" id="editCategoryArea" class="change-pas-box">
  <form class="layui-form" id="fromChangeArea" action="">
           <input type="hidden" name="id" id="rid">
      <div class="layui-form-item">
          <label class="layui-form-label" style="margin-top:15px;">名称:</label>
          <div class="layui-input-block">
              <input type="text" name="categoryName" required lay-verify="required" id="categoryName" class="layui-input" style="margin-top:15px;">
          </div>
      </div>
      <div class="layui-form-item">
          <label class="layui-form-label" style="margin-top:15px;">描述:</label>
            <div class="layui-input-block">
              <textarea style="width:30vw;" name="categoryDescription" id="categoryDescription" placeholder="请输入" class="layui-textarea"></textarea>
            </div>
        </div>
    </form>
</div>
 ...
 <script type="text/html" id="toolbarDemo">
	<div class="layui-btn-container">
	   <button class="layui-btn layui-btn- layui-btn-sm" lay-event="addCategory">
...
<script type="text/javascript">
    ...
    var tableObj = table.render({
            ...
            , id:'userTableDatas'
            , height: 650
            , url: '${APP_PATH}/article/getCategoryPage' //数据接口
            , method: 'GET'
            , title: '文章分类表'
            , page:true//开启分页
            ...
            , cols: [[ //表头
                ...
                ,{ field: 'categoryDescription', title: '分类描述'}
         ...
         function addCategory(){
        	 layer.open({
                 ...
                 ,yes: function(index, layero){
                     ...
                     var nm = $.trim(name);
                     if(nm=='' || nm ==null){
                     	layer.msg("名称不能为空", {time:1000, icon:5}, function(){
                   });
                        return false;
                     }
                     if(nm.length <1 || nm.length > 20){
                     	layer.msg("名称字符长度在1-20", {time:1000, icon:5}, function(){
                    });
                    	   return false;
                     }
                     // ajax提交表单
                     ...
                     var category = {};
                         ...
                         category.categoryDescription =$("#categoryDescription").val();
				    	$.ajax({
				    		type : "POST",
				    		url  : "${APP_PATH}/article/addCategory",
				    		...
				    		success : function(result) {
				    			...
				    			if ( result.success ) {
				    				 layer.msg(result.msg, {time:2000, icon:6, shift:6}, function(){
				    					 layer.close(index); 
				    					 table.reload('userTableDatas', {method: 'GET'});	
				                       });
				    			}else{
			                       layer.msg(result.msg, {time:2000, icon:5, shift:6}, function(){
			                    	   layer.close(index); //如果设定了yes回调，需进行手工关闭
			                            $("#editCategoryArea").css({"display":"none"});
			                       });
				    			}
				    		}
				    	});
        ...
        table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
               ...
               window.location.href="${APP_PATH}/role/goPermissionPage?roleid="+data.id;
                ...
                layer.confirm('确定要删除【'+data.categoryName+'】用户吗?',{icon: 3, title:'提示'}, function (index) {
                    obj.del(); //删除对应行（tr）的DOM结构
                    layer.close(index);
                    //向服务端发送删除指令
                    $.ajax({
         			   type:"post",
             		   url:"${APP_PATH}/article/deleteCategoryById",
            	...
            	$("#categoryDescription").text(data.categoryDescription);
                layer.open({
                    type:1,
                    area:['500px', '300px']
                    ,title:'修改分类'
                    ...
                    ,yes: function(index, layero){
				    	...
				    	$.ajax({
				    		type : "POST",
				    		url  : "${APP_PATH}/article/updateCategory",
				    		data : JSON.stringify(category),
				    		dataType : "json",
				    		contentType:"application/json;UTF-8",
				    		beforeSend : function() {
